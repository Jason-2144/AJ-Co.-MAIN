    ...
    <>
      ...
      <section className="pb-32 container mx-auto px-6">
        <div className="flex flex-wrap justify-center gap-4 mb-16">
            {["All", "E-Commerce", "B2B SaaS", "Professional Services"].map((f, i) => (
               <button key={i} className={`px-6 py-2 rounded-full font-mono text-xs uppercase tracking-widest transition-colors ${i===0 ? 'bg-white text-black font-bold' : 'bg-white/5 text-gray-400 hover:text-white border border-white/10 hover:bg-white/10'}`}>
                  {f}
               </button>
            ))}
        </div>

        <motion.div initial="hidden" whileInView="visible" viewport={{ once:true }} variants={staggerContainer} className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
            {caseStudiesList.map((cs) => (
               <motion.div key={cs.id} variants={fadeUp} className="glass p-10 rounded-2xl border border-white/10 hover:border-[#10B981]/30 transition-all duration-300 flex flex-col group bg-white/[0.02]">
                   ...
                   <h3 className="font-syne text-2xl font-bold text-white mb-6 leading-tight">{cs.title}</h3>
                   <p className="text-gray-400 leading-relaxed mb-10 flex-grow text-sm">
                       {cs.summary}
                   </p>
                   <Link to={`/case-studies/${cs.id}`} className="mt-auto inline-flex items-center gap-3 text-white text-sm font-bold uppercase tracking-wider group-hover:text-emerald-400 transition-colors">
      ...
      <section className="py-32 relative text-center border-t border-white/5 bg-white/[0.01]">
        <div className="container mx-auto px-6 max-w-3xl">
          <h2 className="font-syne text-3xl md:text-4xl font-bold text-white mb-12">Want Results Like This?</h2>
