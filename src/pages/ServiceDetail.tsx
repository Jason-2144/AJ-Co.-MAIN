...
const serviceData: Record<string, any> = {
  "ai-opportunity-assessment": {
    ...
    section2: {
      title: "What We Do",
      cards: [
        { title: "Executive Alignment Workshop", desc: "We facilitate structured sessions with your leadership team to map strategic priorities against AI capabilities. We surface where AI creates leverage — and where it doesn't." },
...
export default function ServiceDetail() {
  ...
  if (!data) return <div className="py-40 text-center text-white">Service not found.</div>;
    ...
    <>
             ...
             {data.section2.badges && (
                <div className="flex flex-wrap gap-4 mt-8">
                  {data.section2.badges.map((b:string, i:number) => (
                    <motion.span key={i} variants={fadeUp} className="px-6 py-3 rounded-full border border-white/10 bg-white/5 text-gray-300 font-mono text-sm tracking-wider">
                      {b}
                    </motion.span>
                  ))}
                </div>
      ...
      <section className="py-32 relative text-center border-t border-white/5 bg-white/[0.01]">
        <div className="container mx-auto px-6 max-w-3xl">
          <h2 className="font-syne text-3xl md:text-4xl font-bold text-white mb-12">{data.cta.headline}</h2>
